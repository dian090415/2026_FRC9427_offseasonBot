...
public void setX() {
    ...
    // 1. 傳送給四個 Swerve Module
    frontLeft.setDesiredState(xStates[0]);
    frontRight.setDesiredState(xStates[1]);
    backLeft.setDesiredState(xStates[2]);
    backRight.setDesiredState(xStates[3]);

    // ✨ 2. 關鍵修復：必須同步更新 currentSetpoint！
    // 讓 SetpointGenerator 知道輪子現在在 45 度，下次切回手動駕駛時才不會發生暴力修正與打轉。
    currentSetpoint = new SwerveSetpoint(new ChassisSpeeds(), xStates);
  }
   ...
   * 判斷機器人目前是否正在爬坡
   *
   * 使用 Pitch / Roll 中較大的傾斜角判斷。
   * 使用 Hysteresis 避免在臨界值附近反覆切換。
   *
   * @return true = 爬坡，false = 平地
   */
  public boolean isClimbing() {

    // 取得目前最大的傾斜角
    double currentTilt = Math.max(
        Math.abs(pigeon.getPitch().getValueAsDouble()),
        Math.abs(pigeon.getRoll().getValueAsDouble()));

    // 超過此角度 → 判定開始爬坡
    double onThreshold = 10.0;

    // 低於此角度 → 判定回到平地
    double offThreshold = 8.0;
  ...
  /**
   * 以指定的旋轉中心進行 Swerve 運動
   *
   * @param speeds        Robot-relative chassis speeds
   * @param pivotLocation 相對於機器人中心的旋轉中心
   */
  public void aimAroundPivot(
      ChassisSpeeds speeds,
      Translation2d pivotLocation) {

    SwerveModuleState[] states = SwerveConstants.kDriveKinematics.toSwerveModuleStates(
        speeds,
        pivotLocation);
  ...
  public void runVelocity(ChassisSpeeds speeds) {
    ...
    // Field-relative → Robot-relative
    ChassisSpeeds robotSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(
        speeds,
        getPose2d().getRotation());

    // 離散化，補償 20ms 控制週期
    ChassisSpeeds discreteSpeeds = ChassisSpeeds.discretize(robotSpeeds, 0.02);
    ...
    // 取得 Setpoint Generator 最終產生的四輪狀態
    SwerveModuleState[] states = currentSetpoint.moduleStates();
