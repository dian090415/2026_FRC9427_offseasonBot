...
package frc.robot.util.VisionHelper;
...
public class LimelightHelpers {
    ...
    // 獲取目標垂直角度 ty
    double ty = getTY("limelight"); 
    // 檢查是否有看到目標 (tv)
    if (!getTV("limelight")) {
    return 0.0; // 沒看到目標時回傳 0，避免 Hood 亂跑
}

    // --- 硬體參數設定 (請根據 2026 機器人實際數值修改) ---
    double h1 = 0.45;  // 你的 Limelight 鏡頭中心離地高度 (公尺)
    double h2 = 1.32;  // 2026 比賽 AprilTag 的中心高度 (公尺)
    double a1 = 25.0;  // Limelight 安裝仰角 (度)
    // --------------------------------------------------

    double angleToGoalRadians = Math.toRadians(a1 + ty);
    
    // 計算水平距離
    double distance = (h2 - h1) / Math.tan(angleToGoalRadians);
    return distance;
}
}